 ...
 */
export function getNextStatement(statements: Statement[], runState: RunState): Statement | null {
  ...
  const allLanesCovered = ALL_LANES.every((lane) => (laneCounts[lane] || 0) >= MIN_LANE_COVERAGE)
  ...
  if (!allLanesCovered) {
    ...
    const needsCoverage = laneCountsArray.filter((item) => item.count < MIN_LANE_COVERAGE)

    // Pick from the lane(s) with lowest count
    const targetLanes = needsCoverage.filter((item) => item.count === needsCoverage[0].count)
  ...
  if (shouldExplore) {
    ...
    const exploreCandidates = unseenStatements.filter((s) => !sortedLanes.includes(s.lane_id))
